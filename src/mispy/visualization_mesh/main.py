...
def draw_face_pair(
    ...
    for face_pair in face_pairs:
        ...
        if text_enable:
            centroid = coords1.mean(axis=0)
            ax.text(centroid[0], centroid[1], centroid[2], f"f_{face_pair[0].glo_id}", color="black", fontsize=10)
            centroid = coords2.mean(axis=0)
            ax.text(centroid[0], centroid[1], centroid[2], f"f_{face_pair[1].glo_id}", color="black", fontsize=10)
        
        if nodes_enable:
            coords = np.array([node.p for node in face_pair[1].nodes + face_pair[0].nodes])
            ids = [node.glo_id for node in face_pair[1].nodes + face_pair[0].nodes]
            draw_nodes(ax = ax, coords = coords, color = "black", texts = ids, text_enable = True)
        ax.set_title(f"Pair num: {num}")
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
